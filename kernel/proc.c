...
found:
  ...
  // Allocate a page for usyscall before we set up the page table for user
  if((p->usyscall = (struct usyscall *)kalloc()) == 0){
    freeproc(p);
    release(&p->lock);
    return 0;
  }
  // store pid to the shared page
...
proc_pagetable(struct proc *p)
{
  ...
  // they are trying to fetch can be prepopulated by the kernel.
